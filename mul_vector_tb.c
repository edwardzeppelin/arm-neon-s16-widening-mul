...
void
mul_vector_tb_q15(long int seed, unsigned runs)
{
  unsigned run;
  uint_fast16_t i, n;

  int32_t * dst = NULL;
  int32_t * dst_ref = NULL;
  int16_t * src1 = NULL;
  int16_t * src2 = NULL;
  ...
  for (run=0; run<runs; run++) {
    n = lrand48() % N_MAX;
    dst = (int32_t *) realloc (dst, n*sizeof(int32_t));
    dst_ref = (int32_t *) realloc (dst_ref, n*sizeof(int32_t));
    ...
    mul_vector_ref_q15(n, dst_ref, src1, src2);
